...
TEST(RunTest, ImageBindingAndExecution) {
    ...
    const std::string img_kernel_code = 
        "__kernel void img_test(__read_only image2d_t src, __write_only image2d_t dest) {\n"
    ...
    // Note: the kernel doesn't take width and height as arguments, so we do not bind them.
    // Reset arg count to be sure.
    run.reset_argcount();
    run.bind_imagef("src", src_data, width, height, Direction::IN);
    run.bind_imagef("dest", dest_data, width, height, Direction::OUT);
